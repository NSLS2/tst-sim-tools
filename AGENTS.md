...
- `src/tst_sim_tools/startup.py` is the real runtime entrypoint: it creates the Bluesky `RunEngine`, subscribes a `TiledWriter` and `BestEffortCallback`, and instantiates the simulated XRT devices.
- Device code is split by abstraction: `devices/detectors.py` contains the XRT screen detector/acquisition/HDF data logic; `devices/mirrors.py` contains the toroid mirror signals.
...
- Queue server tasks are `pixi run -e qs qs-server` and `pixi run -e qs http-server`; `qs-server` writes permissions and device/plan output paths under `/tmp/bsqs`.

## Runtime Gotchas
- `startup.py` reads `TILED` with `int(os.getenv("TILED", ""))`; direct imports without setting `TILED` fail. Use the Pixi startup tasks or set `TILED=0`, `1`, or `2` explicitly.
- `TILED=0` uses local Tiled storage at `/tmp/tst_testing`; `TILED=1` connects to `https://tiled-staging.nsls2.bnl.gov`; `TILED=2` connects to `https://tiled.nsls2.bnl.gov` at `tst/migration`.
